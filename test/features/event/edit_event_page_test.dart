...
void main() {
  ...
  Color? headerColor(WidgetTester tester) {
    final animated = find.ancestor(
      of: find.text('Modifier l’événement'),
  ...
  testWidgets(
      'EditEventPage — le header est transparent au repos et opaque au '
      'scroll', (tester) async {
    router.go('/event/edit?id=event-demo-1');
  ...
  testWidgets('EditEventPage — le bouton « Voir les billets » est absent',
      (tester) async {
    router.go('/event/edit?id=event-demo-1');
    await pumpEdit(tester);

    expect(find.text('Enregistrer les modifications'), findsOneWidget);
    expect(find.text('Voir les billets'), findsNothing);
  });

  testWidgets('EditEventPage — l’encoche du bas est incluse dans le dégagement',
      (tester) async {
